...
import rospy
from darknet_ros_msg import BoundingBoxes
...
class BigRectangleState(EventState):
	...
	def callback(bbox,userdata):
		self.maxProbability = self.threshold
		for box in bbox.bounding_boxes:
			if box.Class == self._target and box.probability >= self.maxProbability:
				...
				if not found:
					self.potential[len(potential)+1].box = box
					self.potential[len(potential)+1].time = bbox.header.stamp.nseq
		return 'Failed'
